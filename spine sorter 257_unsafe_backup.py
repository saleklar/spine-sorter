...
class MainWindow(QMainWindow):
	def __init__(self):
		super().__init__()
		self.setWindowTitle("Spine Sorter v5.71")
		self._setup_icons()
		...
		self.consolidate_duplicates_cb = QCheckBox("Consolidate Duplicate Images")
		self.consolidate_duplicates_cb.setToolTip("If checked, uses only ONE image for duplicates (same content) and remaps JSON references.")
		...
		# Consolidate Mirrored Images option (New)
		self.consolidate_mirrored_cb = QCheckBox("Consolidate Mirrored (Ignore Meshes)")
		self.consolidate_mirrored_cb.setToolTip("If checked, detects flipped/mirrored duplicates (X or Y axis) and reuses the image by setting scaleX/Y = -1. SAFETY: Ignores Mesh attachments to prevent UV distortion.")
		self.consolidate_mirrored_cb.setStyleSheet("font-weight: bold; color: #9C27B0;")
		# Default to False usually as it is aggressive
		self.consolidate_mirrored_cb.setChecked(False)
		actions_layout.addWidget(self.consolidate_mirrored_cb)
	...
	def _process_single_skeleton(self, found_json, found_info, result_dir, folder, input_path, file_scanner, base_output_root, spine_exe, base_progress, name, errors, results, all_file_stats, jpeg_forced_png_warnings, all_skeleton_names=None, is_first=True, is_last=True, optimization_enabled=True, spine_export_unchecked=None, spine_export_unchecked_anims=None, extra_cli_args=None, spine_export_missing=None, spine_export_log_warnings=None, consolidate_duplicates=False, consolidate_mirrored=False):
		...
		try:
			...
			# Import PIL if needed
			if consolidate_mirrored:
				try:
					from PIL import Image
					import io
				except ImportError:
					self.log_warning("Pillow (PIL) not found! Mirror consolidation disabled. Install 'pillow' to enable.")
			...
			if consolidate_mirrored:
				...
				# Optimization: Only generate flips for valid source candidates (e.g. unique hashes)
				# To avoid N^2 work, we iterate UNIQUE real hashes.
				unique_real_hashes = [h for h in content_registry if any(e['type'] == 'real' for e in content_registry[h])]
				
				processed_hashes_for_flip = set()
				
				for h in unique_real_hashes:
					entries = content_registry[h]
					real_entries = [e for e in entries if e['type'] == 'real']
					if not real_entries: continue
					
					# Use the primary (first) real file as the source for virtual flips
					source_path = real_entries[0]['path']
					...
					try:
						...
						with Image.open(source_path) as im:
							...
							# X-Flip
							im_x = im.transpose(Image.FLIP_LEFT_RIGHT)
							buf = io.BytesIO()
							im_x.save(buf, format='PNG')
							h_x = _sha1_bytes(buf.getvalue())
							
							if h_x not in content_registry: content_registry[h_x] = []
							content_registry[h_x].append({'path': source_path, 'type': 'virtual_x', 'hash': h_x})
							
							# Y-Flip
							im_y = im.transpose(Image.FLIP_TOP_BOTTOM)
							buf = io.BytesIO()
							im_y.save(buf, format='PNG')
							h_y = _sha1_bytes(buf.getvalue())
							
							if h_y not in content_registry: content_registry[h_y] = []
							content_registry[h_y].append({'path': source_path, 'type': 'virtual_y', 'hash': h_y})
							
					except Exception as e:
						# self.log_warning(f"Failed to generate flip for {source_path}: {e}")
						pass
			...
			for h, entries in content_registry.items():
				real_entries = [e for e in entries if e['type'] == 'real']
				virtual_entries = [e for e in entries if e['type'].startswith('virtual')]
				...
				# Case B: Mirror Duplicates
				# Logic: Real File A has Hash H. Virtual Entry B (from Source C) also has Hash H.
				# This means A is identical to C-flipped.
				# We can replace A with C (transform).
				if consolidate_mirrored and len(real_entries) > 0 and len(virtual_entries) > 0:
					# Pick best source
					# The virtual entries point to a 'path' (the source image).
					# We want the shortest/cleanest source path.
					virtual_entries.sort(key=lambda x: (len(x['path']), x['path']))
					best_virtual = virtual_entries[0]
					
					target_source = best_virtual['path']
					transform_type = best_virtual['type'] # 'virtual_x' or 'virtual_y'
					axis = 'x' if 'x' in transform_type else 'y'
					
					# Targets to replace: All real files in this bucket
					for entry in real_entries:
						target_path = entry['path']
						
						# Skip if it is the source itself (shouldn't happen unless symmetric)
						if os.path.normcase(os.path.abspath(target_path)) == os.path.normcase(os.path.abspath(target_source)):
							continue
							
						norm_t = os.path.normcase(os.path.abspath(target_path))
						
						# Exact consolidation takes precedence over mirror (simpler)
						if norm_t in consolidation_map:
							continue
							
						# Add to mirror map
						consolidation_map_mirror[norm_t] = {
			...
			if consolidation_map_mirror:
				...
				for k, v in consolidation_map_mirror.items():
					if count < 5:
						msg = f"  <font color='#9C27B0'>Mirror: {os.path.basename(k)} -> {os.path.basename(v['target'])} (Flip {v['axis'].upper()})</font>"
					...
					for slot_name, attachments in list(skin_dict.items()):
						...
						for attach_name, attach_val in list(attachments.items()):
							...
							try:
								...
								# 2. Mirror Consolidation (New)
								if src and isinstance(src, str) and consolidation_map_mirror and isinstance(attach_val, dict):
									norm_p = os.path.normcase(os.path.abspath(src))
									if norm_p in consolidation_map_mirror:
										...
										if not is_complex:
											...
											if not scan_mode:
												...
												if axis == 'x':
													current_sq = attach_val.get('scaleX', 1.0)
													attach_val['scaleX'] = current_sq * -1.0
													# Also check rotation? Usually flip is enough.
												elif axis == 'y':
													current_sq = attach_val.get('scaleY', 1.0)
													attach_val['scaleY'] = current_sq * -1.0
		...
		for i, item_data in enumerate(to_process):
			...
			for idx, f_json in enumerate(found_jsons):
				...
				self._process_single_skeleton(
					...
					consolidate_mirrored=self.consolidate_mirrored_cb.isChecked()
				)
