...
class MainWindow(QMainWindow):
	...
	def process_selected(self):
		...
		for name in to_process:
			...
			result_dir = os.path.join(base_output_root, f"spine_temp_{timestamp}_{to_process.index(name)}")
			...
			found_info = None
			for _ in range(15):
				current_jsons = [os.path.join(result_dir, f) for f in os.listdir(result_dir) if f.lower().endswith('.json')]
				if current_jsons:
					found_jsons = current_jsons
					# Wait a bit more to ensure all are written
					time.sleep(0.5)
					# Re-check
					found_jsons = [os.path.join(result_dir, f) for f in os.listdir(result_dir) if f.lower().endswith('.json')]
					break
				time.sleep(1)
			...
			if not found_jsons:
				...
				if os.path.exists(alt_json):
					found_jsons.append(alt_json)

			if not found_jsons:
				msg = f"No exported json for {name} (expected in {result_dir})"
				errors.append(msg)
			...
			for found_json in found_jsons:
				self._process_single_skeleton(found_json, found_info, result_dir, folder, input_path, None, base_output_root, spine_exe, None, name, errors, results, all_file_stats, jpeg_forced_png_warnings)
