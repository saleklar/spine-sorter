#!/usr/bin/env python3
"""
Spine Sorter v5.54 - PySide6 UI for managing Spine Animation Files
...
class MainWindow(QMainWindow):
	def __init__(self):
		super().__init__()
		self.setWindowTitle("Spine Sorter v5.54")
		self._setup_icons()
	...
	def _process_single_skeleton(self, found_json, found_info, result_dir, folder, input_path, file_scanner, base_output_root, spine_exe, base_progress, name, errors, results, all_file_stats, jpeg_forced_png_warnings, all_skeleton_names=None, is_first=True, is_last=True, optimization_enabled=True, spine_export_unchecked=None, spine_export_unchecked_anims=None):
		...
		# Record resolved image paths for this skeleton (normalized absolute paths)
		try:
			norm_resolved = [os.path.normcase(os.path.abspath(p)) for p in resolved]
			if all_file_stats:
				all_file_stats[-1]['resolved_image_paths'] = norm_resolved
		except Exception:
			pass
		try:
			if resolved:
	...
	def process_selected(self):
		...
		try:
			...
			# Build usage map: normalized path -> set of skeleton/file names that reference it
			usage_map = {}
			for stats in all_file_stats:
				owners = stats.get('name')
				for p in stats.get('resolved_image_paths', []) or []:
					usage_map.setdefault(p, set()).add(owners)

			per_file_recs = []
			for name, groups in dup_by_file.items():
				# For each duplicate group, only report it if ALL files in the group are used exclusively
				# by this single skeleton/file. If any file is shared across multiple skeletons/files,
				# skip reporting that group.
				reported_any = False
				for g in groups:
					...
					for p in g:
						try:
							...
							norm_group.append(np)
						except Exception:
							norm_group.append(p)
					# collect owners for all paths in group
					owners_union = set()
					for np in norm_group:
						owners_union.update(usage_map.get(np, set()))
					# If owners_union contains exactly one owner (this skeleton/file), and that owner
					# is the current `name`, then report; otherwise skip.
					if len(owners_union) == 1 and (name in owners_union):
						if not reported_any:
							per_file_recs.append(f"Identical images detected in file: {name}")
							reported_any = True
							per_file_recs.append(" - " + " | ".join(g))
					# else: skip group because files are shared across skeletons/files
				if reported_any:
